...
try {
    ...
    app.use(async (ctx, next) => {
        ...
        if (!value || value.operationName === "IntrospectionQuery") {
          ...
          const validQuery: string[] = ["addUser", "login", "logout"]
          const administratorValidQuery: string[] = ["getJourneys", "getCars", "getDrivers", "getClients"]
          const driverValidQuery: string[] = ["addCar", "setAvailability"]
          ...
          if (value && validQuery.some((elem) => value.query.includes(elem))) {
            await next()
          } else if (value && value.query.includes("rentCar")) {
            if (user) {
              if (user.role !== "CLIENT") {
                ctx.response.status = 401;
                ctx.response.body = { error: "You are not a client" };
              } else {
                ctx.state.user = user;
                await next();
              }
          ...
          } else if (value && driverValidQuery.some((elem) => value.query.includes(elem))) {
            if (user) {
              if (user.role !== "DRIVER") {
                ctx.response.status = 401;
                ctx.response.body = { error: "You are not a driver" };
              } else {
                ctx.state.user = user
                await next()
              }
          ...
          } else if (value && administratorValidQuery.some((elem) => value.query.includes(elem))) {
            if (user) {
              if (user.role !== "ADMIN") {
                ctx.response.status = 401;
